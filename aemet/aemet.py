...
# Additional location of the entity
...
class AemetAPI:
    ...
    def aemet_load_from_file(self, api_url):
        ...
        if file is None:
            raise HomeAssistantError(
                "Can't find API method for {}".format(api_url)
            )
    ...
    def api_call(self, api_url: str, direct_call: bool = False, cached: bool = False):
        ...
        if not direct_call:
            ...
            if estado is None:
                ...
                _LOGGER.info("Could not retrieve data from AEMET. Error %s, %s", estado, descripcion)
...
class AemetMasterRecord:
    ...
    def _write_master_data(self, force=False):
        ...
        last_saved = datetime.strptime(self.data.get('saved', datetime.now()), '%Y-%m-%dT%H:%M:%S')
    ...
    def _clean_master_data(self, data):
        ...
        clean_data = []
        for entity in data:
            clean_entity = {MAP_FIELDS[k]: v for k, v in entity.items() if k in MAP_FIELDS}
            clean_entity['codigo'] = clean_entity['codigo'][2:] if clean_entity['codigo'].startswith('id') else \
                clean_entity['codigo']
            clean_entity = {k: (float(v) if k in FLOAT_FIELDS else v) for k, v in clean_entity.items()}
...
class AemetForecast:
        ...
        def inner_transform(
            ...
            for field in fields:
                ...
                if resultado is None:
                    ...
                    if resultado != "": transformed_data[interval][name] = resultado
    ...
    def update(self, nearest=None, api_client=None):
        ...
        self.data = {self._forecastmode: data, "saved": (
            datetime.now()
                .replace(microsecond=0)
                .isoformat()
        )}
...
class AemetWeather:
    ...
    def update(self, nearest=None, api_client=None):
        ...
        self.data = {'currently': data, "saved": (
            datetime.now()
                .replace(microsecond=0)
                .isoformat()
        )}
...
class AemetData:
    ...
    def __init__(self, latitude, longitude, elevation=0, api_key=None, cache_dir=DEFAULT_CACHE_DIR, weather_station=None):
        """Initialize the data object."""
        ...
        self.weather_stations = AemetMasterRecord('estaciones', self.api_client, weather_station=weather_station)
    ...
    def update(self):
        ...
        self.weather_stations.update_distance(self.location)
        self.currently.update(nearest=self.weather_stations.nearest, api_client=self.api_client)
        ...
        self.data = {'currently': self.currently.data['currently'], "hourly": self.hourly.data['horaria'], "daily": self.daily.data['diaria'], "saved": (
            datetime.now()
                .replace(microsecond=0)
                .isoformat()
        )}
